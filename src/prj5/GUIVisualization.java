...
/**
 * GUI front-end for the Social Media Visualization project. Displays a bar
 * chart of influencer engagement rates with buttons to control time period,
 * sort order, and formula.
 *
...
public class GUIVisualization
{
    ...
    private static final int GRAPH_LEFT = 80;
    private static final int GRAPH_TOP = 60;
    private static final int GRAPH_WIDTH = 720;
    private static final int GRAPH_HEIGHT = 380;

    private static final int BAR_WIDTH = 80;
    private static final int BAR_GAP = 100;

    private static final DecimalFormat FORMATTER = new DecimalFormat("#.#");

    // Bar colors for each channel (up to 4)
    private static final Color[] BAR_COLORS = { new Color(70, 130, 180),   // steel
                                                                           // blue
    ...
    private String currentMonth;      // "January", "February", "March", "First
                                      // Quarter"
     ...
     *
     * @param data
     *            the influencer data list read from the input file
     */
    ...
    private SinglyLinkedList<ChannelData> buildChannelData()
    {
        ...
        for (int i = 0; i < allData.size(); i++)
        {
            ...
            ChannelData existing = findChannel(channels, row.getChannelName());
    ...
    private
        ChannelData
        findChannel(SinglyLinkedList<ChannelData> list, String channelName)
    {
    ...
    private void drawBars(SinglyLinkedList<ChannelData> channels)
    {
        ...
        int totalBarWidth =
            count * BAR_WIDTH + (count - 1) * (BAR_GAP - BAR_WIDTH);
        int startX = GRAPH_LEFT + (GRAPH_WIDTH - totalBarWidth) / 2;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            int barX = startX + i * BAR_GAP;
            int barHeight =
                (rate > 0) ? (int)(rate / maxRate * (GRAPH_HEIGHT - 40)) : 0;
            ...
            {
                Shape bar =
                    new Shape(barX, barY, BAR_WIDTH, barHeight, barColor);
                window.addShape(bar);
                drawnBars.add(bar);
            }
            ...
            TextShape nameLabel =
                new TextShape(0, 0, channel.getChannelName(), Color.BLACK);
            int nameLabelX = barX + BAR_WIDTH / 2 - nameLabel.getWidth() / 2;
            ...
            TextShape rateText =
                new TextShape(0, 0, rateLabel, Color.DARK_GRAY);
            int rateLabelX = barX + BAR_WIDTH / 2 - rateText.getWidth() / 2;
    ...
    {
        String title =
            currentMonth + " | " + currentFormula + " | Sort: " + currentSort;
        TextShape titleShape = new TextShape(0, 0, title, Color.BLACK);
        int tx = WINDOW_WIDTH / 2 - titleShape.getWidth() / 2;
        titleShape.setX(tx);
        titleShape.setY(GRAPH_TOP - 40);
